...
const loadModule = (cmpMeta, hostRef, hmrVersionId) => {
    ...
    return __sc_import_findmyservice(
...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return __sc_import_findmyservice('./css-shim-229ebf7a-229ebf7a.js');
    }
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await __sc_import_findmyservice('./dom-a0c82e31-a0c82e31.js');
        }
...
const proxyComponent = (Cstr, cmpMeta, flags) => {
    if (BUILD.member && cmpMeta.$members$) {
        ...
        members.forEach(([memberName, [memberFlags]]) => {
            if ((BUILD.prop && (memberFlags & 31 /* Prop */)) || (BUILD.state && (!BUILD.lazyLoad || flags & 2 /* proxyState */) && (memberFlags & 32 /* State */))) {
                // proxyComponent - prop
...
const initializeDocumentHydrate = (node, rootOriginalLocations) => {
    if (node.nodeType === 1 /* ElementNode */) {
        let i = 0;
        for (; i < node.childNodes.length; i++) {
            initializeDocumentHydrate(node.childNodes[i], rootOriginalLocations);
        }
        if (node.shadowRoot) {
            for (i = 0; i < node.shadowRoot.childNodes.length; i++) {
                initializeDocumentHydrate(node.shadowRoot.childNodes[i], rootOriginalLocations);
            }
        }
    }
    else if (node.nodeType === 8 /* CommentNode */) {
    ...
    if ((BUILD.lazyLoad || BUILD.style) && (hostRef.$flags$ & 256 /* hasInitializedComponent */) === 0) {
        ...
        if (BUILD.style && !Cstr.isStyleRegistered && Cstr.style) {
            ...
            if (!BUILD.hydrateServerSide && BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await __sc_import_findmyservice('./shadow-css-984bac74-549b16dd.js').then(m => m.scopeCss(style, scopeId, false));
            }
...
const bootstrapLazy = (lazyBundles, options = {}) => {
    ...
    if (BUILD.hydrateClientSide && BUILD.shadowDom) {
        ...
        styles.forEach(styleElm => {
            registerStyle(styleElm.getAttribute(HYDRATE_ID), globalStyles + convertScopedToShadow(styleElm.innerHTML));
        });
    }
...
export { patchBrowser as a, globals as b, bootstrapLazy as c, patchEsm as d, registerInstance as e, h as f };
